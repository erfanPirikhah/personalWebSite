      ...
      <section className="bg-homeBg dark:bg-homeTwoBg-dark min-h-screen  bg-no-repeat bg-center bg-cover bg-fixed  md:pb-16 w-full">
            ...
            <div className="flex items-center">
              ...
              {!menuOpen ? (
                <span
                  ...
                  className="lg:hidden bg-[#ef4060] w-[40px] h-[40px] rounded-full flex justify-center items-center text-white dark:text-white text-3xl mr-3 "
                >
                  <AiOutlineMenu />
                </span>
              ) : (
                <span
                  onClick={() => setMenuOpen(!menuOpen)}
                  className="lg:opacity-0 lg:invisible visible opacity-100  bg-[#ef4060] w-[40px] h-[40px] rounded-full flex justify-center items-center text-white text-3xl mr-3 "
                >
                  <AiOutlineClose />
                </span>
              )}
        ...
        <nav className={`${menuOpen ? "block lg:hidden" : "hidden"}`}>
          ...
          <ul
            className={`${
              menuOpen
                ? "block  rounded-b-[20px] shadow-md absolute right-0 top-20 z-[22222222222222] w-full bg-white dark:bg-[#1d1d1d]"
                : "flex my-12 "
            }`}
          >
            {menuItemTwo.map((item) => (
              <li onClick={() => setMenuOpen(false)} key={item.id}>
                <NavLink
                  ...
                  className={`${
                    menuOpen ? " pr-4" : " mx-2.5 rounded-md "
